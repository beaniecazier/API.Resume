...
public class UpdatePhoneNumberModelEndpoint : IEndpoints
{
	...
	/// <param name="countryCodeService"></param>
    ...
    /// <returns>The updated copy of the PhoneNumber Model</returns>
    ...
    private static async Task<IResult> UpdatePhoneNumberModelAsync(int id, UpdatePhoneNumberModelRequest changes,
        IPhoneNumberModelService service, ICountryCodeModelService countryCodeService,
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        if (oldModel.IsFail)
        {
            Log.Error(((Error)oldModel).ToException(), "Server issue encountered while trying to get all PhoneNumber Models from the database");
            return Results.Problem(detail: ((Error)oldModel).ToException().ToString(), statusCode: StatusCodes.Status500InternalServerError);
        }      
        var requestedCountryCodeModel = await countryCodeService.GetByIDAsync(changes.CountryCode.Value, token);
        if (requestedCountryCodeModel.IsFail)
        {
            Log.Error(((Error)requestedCountryCodeModel).ToException(), "Server issue encountered while trying to query for the list of requested ***PLURALIZE***");
            return Results.Problem(detail: ((Error)requestedCountryCodeModel).ToException().ToString(), statusCode: StatusCodes.Status500InternalServerError);
        }

        var newModel = changes.MapToModelFromUpdateRequest(username, (CountryCodeModel)requestedCountryCodeModel);
        ...
        return result.Match(
            Succ =>
            {
                Log.Information("PhoneNumber Model with id {model.CommonIdentity} was successfully updated", @newModel.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetPhoneNumberModelByID", new { id = newModel.CommonIdentity });
