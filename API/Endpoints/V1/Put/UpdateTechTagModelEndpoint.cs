...
public class UpdateTechTagModelEndpoint : IEndpoints
{
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        var singleEndpoint = app.MapPut(TechTagModelEndpoints.Put, UpdateTechTagModelAsync)
            .WithName("UpdateTechTagModel")
            .Accepts<UpdateTechTagModelRequest>(ContentType)
    ...
    private static async Task<IResult> UpdateTechTagModelAsync(int id, UpdateTechTagModelRequest changes,
        ...
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        if (!result.IsFail) await outputCacheStore.EvictByTagAsync(TechTagModelEndpoints.Tag, token);
        return result.Match(
            Succ =>
            {
