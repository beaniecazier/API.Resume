...
/// </summary>
[ApiVersion(1.0)]
public class UpdateOrganizationModelEndpoint : IEndpoints
{
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        // Update Endpoints
        Log.Information("Now adding Organization Model put Endpoints");
        var singleEndpoint = app.MapPut(OrganizationModelEndpoints.Put, UpdateOrganizationModelAsync)
            .WithName("UpdateOrganizationModel")
            .Accepts<UpdateOrganizationModelRequest>(ContentType)
    ...
    /// <summary>
    /// Search for and remove a specific Organization Model result from the database
    /// </summary>
    /// <param name="id"></param>
    ...
    /// <returns>The updated copy of the Organization Model</returns>
    ...
    private static async Task<IResult> UpdateOrganizationModelAsync(int id, UpdateOrganizationModelRequest changes,
        IOrganizationModelService service, IAddressModelService addressService, IPhoneNumberModelService phoneNumberService,
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        Log.Information("Update Organization Model Endpoint called by {username}", @username);
        ...
        {
            Log.Error("Organization Model with ID:{id} does not exist", @changes.Id);
            return Results.NotFound();
        }
        if (oldModel.IsFail)
        {
            Log.Error(((Error)oldModel).ToException(), "Server issue encountered while trying to get all Organization Models from the database");
        ...
        var result = await service.UpdateAsync(newModel, (OrganizationModel)oldModel, token);
        if (!result.IsFail) await outputCacheStore.EvictByTagAsync(OrganizationModelEndpoints.Tag, token);
        return result.Match(
            Succ =>
            {
                Log.Information("Organization Model with id {model.CommonIdentity} was successfully updated", @newModel.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetOrganizationModelByID", new { id = newModel.CommonIdentity });
