...
/// </summary>
[ApiVersion(1.0)]
public class UpdateCountryCodeModelEndpoint : IEndpoints
{
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        // Update Endpoints
        Log.Information("Now adding CountryCode Model put Endpoints");
        var singleEndpoint = app.MapPut(CountryCodeModelEndpoints.Put, UpdateCountryCodeModelAsync)
            .WithName("UpdateCountryCodeModel")
            .Accepts<UpdateCountryCodeModelRequest>(ContentType)
    ...
    /// <summary>
    /// Search for and remove a specific CountryCode Model result from the database
    /// </summary>
    /// <param name="id"></param>
    ...
    /// <returns>The updated copy of the CountryCode Model</returns>
    ...
    private static async Task<IResult> UpdateCountryCodeModelAsync(int id, UpdateCountryCodeModelRequest changes,
        ICountryCodeModelService service,
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        Log.Information("Update CountryCode Model Endpoint called by {username}", @username);
        ...
        {
            Log.Error("CountryCode Model with ID:{id} does not exist", @changes.Id);
            return Results.NotFound();
        }
        if (oldModel.IsFail)
        {
            Log.Error(((Error)oldModel).ToException(), "Server issue encountered while trying to get all CountryCode Models from the database");
            return Results.Problem(detail: ((Error)oldModel).ToException().ToString(), statusCode: StatusCodes.Status500InternalServerError);
        }      

        var newModel = changes.MapToModelFromUpdateRequest(username);
        ...
        var result = await service.UpdateAsync(newModel, (CountryCodeModel)oldModel, token);
        if (!result.IsFail) await outputCacheStore.EvictByTagAsync(CountryCodeModelEndpoints.Tag, token);
        return result.Match(
            Succ =>
            {
                Log.Information("CountryCode Model with id {model.CommonIdentity} was successfully updated", @newModel.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetCountryCodeModelByID", new { id = newModel.CommonIdentity });
