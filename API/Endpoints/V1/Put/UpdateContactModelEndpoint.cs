...
/// <summary>
/// The collection of Endpoints for the Contact Model in API
/// </summary>
[ApiVersion(1.0)]
public class UpdateContactModelEndpoint : IEndpoints
{
    private const string ContentType = "application/json";

    /// <summary>
    /// Add the Contact Model Service to the DI container
    /// </summary>
    ...
    /// <summary>
    /// Map all Contact Model Endpoints with correct settings
    /// </summary>
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        // Update Endpoints
        Log.Information("Now adding Contact Model put Endpoints");
        var singleEndpoint = app.MapPut(ContactModelEndpoints.Put, UpdateContactModelAsync)
            .WithName("UpdateContactModel")
            .Accepts<UpdateContactModelRequest>(ContentType)
            ...
            .HasApiVersion(1.0)
            .WithTags(ContactModelEndpoints.Tag);
    ...
    /// <summary>
    /// Search for and remove a specific Contact Model result from the database
    /// </summary>
    /// <param name="id"></param>
    ...
    /// <returns>The updated copy of the Contact Model</returns>
    ...
    private static async Task<IResult> UpdateContactModelAsync(int id, UpdateContactModelRequest changes,
        IContactModelService service, IPhoneNumberModelService phoneNumberService,
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        Log.Information("Update Contact Model Endpoint called by {username}", @username);
        ...
        {
            Log.Error("Contact Model with ID:{id} does not exist", @changes.Id);
            return Results.NotFound();
        }
        if (oldModel.IsFail)
        {
            Log.Error(((Error)oldModel).ToException(), "Server issue encountered while trying to get all Contact Models from the database");
        ...
        var newModel = changes.MapToModelFromUpdateRequest(username, (List<PhoneNumberModel>)requestedPhoneNumberModels);
        ...
        var result = await service.UpdateAsync(newModel, (ContactModel)oldModel, token);
        if (!result.IsFail) await outputCacheStore.EvictByTagAsync(ContactModelEndpoints.Tag, token);
        return result.Match(
            Succ =>
            {
                Log.Information("Contact Model with id {model.CommonIdentity} was successfully updated", @newModel.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetContactModelByID", new { id = newModel.CommonIdentity });
