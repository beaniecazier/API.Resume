...
public class UpdateResumeModelEndpoint : IEndpoints
{
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        var singleEndpoint = app.MapPut(ResumeModelEndpoints.Put, UpdateResumeModelAsync)
            .WithName("UpdateResumeModel")
            .Accepts<UpdateResumeModelRequest>(ContentType)
	...
	/// <param name="educationDegreeService"></param>
	/// <param name="certificateService"></param>
	/// <param name="workExperienceService"></param>
    ...
    private static async Task<IResult> UpdateResumeModelAsync(int id, UpdateResumeModelRequest changes,
        ...
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        var newModel = changes.MapToModelFromUpdateRequest(username,
            (List<EducationDegreeModel>)requestedEducationDegreeModels,
            (List<CertificateModel>)requestedCertificateModels,
            (List<WorkExperienceModel>)requestedWorkExperienceModels,
            (List<ProjectModel>)requestedProjectModels,
            (List<AddressModel>)requestedAddressModels,
            (List<PhoneNumberModel>)requestedPhoneNumberModels);
        ...
        if (!result.IsFail) await outputCacheStore.EvictByTagAsync(ResumeModelEndpoints.Tag, token);
        return result.Match(
            Succ =>
            {
