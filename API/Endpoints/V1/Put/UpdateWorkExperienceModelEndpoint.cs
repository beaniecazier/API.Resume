...
/// </summary>
[ApiVersion(1.0)]
public class UpdateWorkExperienceModelEndpoint : IEndpoints
{
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        // Update Endpoints
        Log.Information("Now adding WorkExperience Model put Endpoints");
        var singleEndpoint = app.MapPut(WorkExperienceModelEndpoints.Put, UpdateWorkExperienceModelAsync)
            .WithName("UpdateWorkExperienceModel")
            .Accepts<UpdateWorkExperienceModelRequest>(ContentType)
    ...
    /// <summary>
    /// Search for and remove a specific WorkExperience Model result from the database
    /// </summary>
    /// <param name="id"></param>
	...
	/// <param name="techTagService"></param>
    ...
    private static async Task<IResult> UpdateWorkExperienceModelAsync(int id, UpdateWorkExperienceModelRequest changes,
        IWorkExperienceModelService service, IOrganizationModelService organizationService, ITechTagModelService techTagService,
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        Log.Information("Update WorkExperience Model Endpoint called by {username}", @username);
        ...
        {
            Log.Error("WorkExperience Model with ID:{id} does not exist", @changes.Id);
            return Results.NotFound();
        }
        if (oldModel.IsFail)
        {
            Log.Error(((Error)oldModel).ToException(), "Server issue encountered while trying to get all WorkExperience Models from the database");
            return Results.Problem(detail: ((Error)oldModel).ToException().ToString(), statusCode: StatusCodes.Status500InternalServerError);
        }      
        var requestedOrganizationModel = await organizationService.GetByIDAsync(changes.Company.Value, token);
        if (requestedOrganizationModel.IsFail)
        {
            Log.Error(((Error)requestedOrganizationModel).ToException(), "Server issue encountered while trying to query for the list of requested ***PLURALIZE***");
            return Results.Problem(detail: ((Error)requestedOrganizationModel).ToException().ToString(), statusCode: StatusCodes.Status500InternalServerError);
        }

        var requestedTechTagModels = await techTagService.GetAllAsync(new GetAllTechTagModelsOptions {SpecificIds = changes.TechUsed}, token);
        ...
        var newModel = changes.MapToModelFromUpdateRequest(username, (OrganizationModel)requestedOrganizationModel, (List<TechTagModel>)requestedTechTagModels);
        ...
        var result = await service.UpdateAsync(newModel, (WorkExperienceModel)oldModel, token);
        if (!result.IsFail) await outputCacheStore.EvictByTagAsync(WorkExperienceModelEndpoints.Tag, token);
        return result.Match(
            Succ =>
            {
                Log.Information("WorkExperience Model with id {model.CommonIdentity} was successfully updated", @newModel.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetWorkExperienceModelByID", new { id = newModel.CommonIdentity });
