...
public class UpdateCertificateModelEndpoint : IEndpoints
{
        ...
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        var requestedOrganizationModel = await organizationService.GetByIDAsync(changes.Issuer.Value, token);
        ...
        return result.Match(
            Succ =>
            {
                Log.Information("Certificate Model with id {model.CommonIdentity} was successfully updated", @newModel.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetCertificateModelByID", new { id = newModel.CommonIdentity });
