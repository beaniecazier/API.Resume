...
/// </summary>
[ApiVersion(1.0)]
public class UpdateEducationDegreeModelEndpoint : IEndpoints
{
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        // Update Endpoints
        Log.Information("Now adding EducationDegree Model put Endpoints");
        var singleEndpoint = app.MapPut(EducationDegreeModelEndpoints.Put, UpdateEducationDegreeModelAsync)
            .WithName("UpdateEducationDegreeModel")
            .Accepts<UpdateEducationDegreeModelRequest>(ContentType)
    ...
    /// <summary>
    /// Search for and remove a specific EducationDegree Model result from the database
    /// </summary>
    /// <param name="id"></param>
    ...
    /// <returns>The updated copy of the EducationDegree Model</returns>
    ...
    private static async Task<IResult> UpdateEducationDegreeModelAsync(int id, UpdateEducationDegreeModelRequest changes,
        IEducationDegreeModelService service, IOrganizationModelService organizationService,
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        {
            Log.Error("EducationDegree Model with ID:{id} does not exist", @changes.Id);
        ...
        var requestedOrganizationModel = await organizationService.GetByIDAsync(changes.Institution.Value, token);
        ...
        var newModel = changes.MapToModelFromUpdateRequest(username, (OrganizationModel)requestedOrganizationModel);
        ...
        var result = await service.UpdateAsync(newModel, (EducationDegreeModel)oldModel, token);
        if (!result.IsFail) await outputCacheStore.EvictByTagAsync(EducationDegreeModelEndpoints.Tag, token);
        return result.Match(
            Succ =>
            {
                Log.Information("EducationDegree Model with id {model.CommonIdentity} was successfully updated", @newModel.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetEducationDegreeModelByID", new { id = newModel.CommonIdentity });
