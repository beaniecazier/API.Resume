...
/// </summary>
[ApiVersion(1.0)]
public class CreateContactModelEndpoint : IEndpoints
{
    ...
    {
        // Create Endpoints
        Log.Information("Now adding Contact Model post Endpoints");
        var singleEndpoint = app.MapPost(ContactModelEndpoints.Post, CreateContactModelAsync)
            .WithName("CreateContact")
            .Accepts<ContactModel>(ContentType)
            .Produces<ContactModel>(StatusCodes.Status201Created)
    ...
    /// </summary>
    /// <param name="request">The parameters used to make the new Contact Model</param>
    ...
    private static async Task<IResult> CreateContactModelAsync(CreateContactModelRequest request,
        ...
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        Log.Information("Create Contact Model Endpoint called by {username}", @username);
        ...
        var model = request.MapToModelFromCreateRequest(id, username, (List<PhoneNumberModel>)requestedPhoneNumberModels);
        ...
        if(!created.IsFail) await outputCacheStore.EvictByTagAsync(ContactModelEndpoints.Tag, token);
        return created.Match(
            succ =>
            {
                Log.Information("Contact Model Created with id {model.CommonIdentity}", @model.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetContactModelByID", new { id = model.CommonIdentity });
                return Results.Created(locationUri, succ);
            },
            fail =>
            {
                Log.Error(fail.ToException(), "Server issue encountered while trying to add a new Contact Model to the database");
