...
/// </summary>
[ApiVersion(1.0)]
public class CreateOrganizationModelEndpoint : IEndpoints
{
    ...
    {
        // Create Endpoints
        Log.Information("Now adding Organization Model post Endpoints");
        var singleEndpoint = app.MapPost(OrganizationModelEndpoints.Post, CreateOrganizationModelAsync)
            .WithName("CreateOrganization")
            .Accepts<OrganizationModel>(ContentType)
            .Produces<OrganizationModel>(StatusCodes.Status201Created)
    ...
    /// </summary>
    /// <param name="request">The parameters used to make the new Organization Model</param>
    ...
    private static async Task<IResult> CreateOrganizationModelAsync(CreateOrganizationModelRequest request,
        ...
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        Log.Information("Create Organization Model Endpoint called by {username}", @username);
        ...
        if(!created.IsFail) await outputCacheStore.EvictByTagAsync(OrganizationModelEndpoints.Tag, token);
        return created.Match(
            succ =>
            {
                Log.Information("Organization Model Created with id {model.CommonIdentity}", @model.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetOrganizationModelByID", new { id = model.CommonIdentity });
                return Results.Created(locationUri, succ);
            },
            fail =>
            {
                Log.Error(fail.ToException(), "Server issue encountered while trying to add a new Organization Model to the database");
