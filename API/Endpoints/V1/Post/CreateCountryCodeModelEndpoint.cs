...
/// </summary>
[ApiVersion(1.0)]
public class CreateCountryCodeModelEndpoint : IEndpoints
{
    ...
    {
        // Create Endpoints
        Log.Information("Now adding CountryCode Model post Endpoints");
        var singleEndpoint = app.MapPost(CountryCodeModelEndpoints.Post, CreateCountryCodeModelAsync)
            .WithName("CreateCountryCode")
            .Accepts<CountryCodeModel>(ContentType)
            .Produces<CountryCodeModel>(StatusCodes.Status201Created)
    ...
    /// <summary>
    /// Create a new CountryCode Model
    /// </summary>
    /// <param name="request">The parameters used to make the new CountryCode Model</param>
    ...
    private static async Task<IResult> CreateCountryCodeModelAsync(CreateCountryCodeModelRequest request,
        ...
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        Log.Information("Create CountryCode Model Endpoint called by {username}", @username);
        ...
        if(!created.IsFail) await outputCacheStore.EvictByTagAsync(CountryCodeModelEndpoints.Tag, token);
        return created.Match(
            succ =>
            {
                Log.Information("CountryCode Model Created with id {model.CommonIdentity}", @model.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetCountryCodeModelByID", new { id = model.CommonIdentity });
                return Results.Created(locationUri, succ);
            },
            fail =>
            {
                Log.Error(fail.ToException(), "Server issue encountered while trying to add a new CountryCode Model to the database");
