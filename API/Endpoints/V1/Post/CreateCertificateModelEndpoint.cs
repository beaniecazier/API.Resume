...
/// </summary>
[ApiVersion(1.0)]
public class CreateCertificateModelEndpoint : IEndpoints
{
    ...
    {
        // Create Endpoints
        Log.Information("Now adding Certificate Model post Endpoints");
        var singleEndpoint = app.MapPost(CertificateModelEndpoints.Post, CreateCertificateModelAsync)
            .WithName("CreateCertificate")
            .Accepts<CertificateModel>(ContentType)
            .Produces<CertificateModel>(StatusCodes.Status201Created)
    ...
    /// </summary>
    /// <param name="request">The parameters used to make the new Certificate Model</param>
    ...
    private static async Task<IResult> CreateCertificateModelAsync(CreateCertificateModelRequest request,
        ICertificateModelService service, IOrganizationModelService organizationService,
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        Log.Information("Create Certificate Model Endpoint called by {username}", @username);

        int id = await service.GetNextAvailableId();
        
        var requestedOrganizationModel = await organizationService.GetByIDAsync(request.Issuer, token);
        ...
        var model = request.MapToModelFromCreateRequest(id, username, (OrganizationModel)requestedOrganizationModel!);
        ...
        if(!created.IsFail) await outputCacheStore.EvictByTagAsync(CertificateModelEndpoints.Tag, token);
        return created.Match(
            succ =>
            {
                Log.Information("Certificate Model Created with id {model.CommonIdentity}", @model.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetCertificateModelByID", new { id = model.CommonIdentity });
                return Results.Created(locationUri, succ);
            },
            fail =>
            {
                Log.Error(fail.ToException(), "Server issue encountered while trying to add a new Certificate Model to the database");
