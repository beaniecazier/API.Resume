...
/// </summary>
[ApiVersion(1.0)]
public class CreateTechTagModelEndpoint : IEndpoints
{
    ...
    {
        // Create Endpoints
        Log.Information("Now adding TechTag Model post Endpoints");
        var singleEndpoint = app.MapPost(TechTagModelEndpoints.Post, CreateTechTagModelAsync)
            .WithName("CreateTechTag")
            .Accepts<TechTagModel>(ContentType)
            .Produces<TechTagModel>(StatusCodes.Status201Created)
    ...
    /// </summary>
    /// <param name="request">The parameters used to make the new TechTag Model</param>
    ...
    private static async Task<IResult> CreateTechTagModelAsync(CreateTechTagModelRequest request,
        ITechTagModelService service,
        IOutputCacheStore outputCacheStore, LinkGenerator linker, HttpContext http, CancellationToken token)
    {
        ...
        Log.Information("Create TechTag Model Endpoint called by {username}", @username);
        ...
        if(!created.IsFail) await outputCacheStore.EvictByTagAsync(TechTagModelEndpoints.Tag, token);
        return created.Match(
            succ =>
            {
                Log.Information("TechTag Model Created with id {model.CommonIdentity}", @model.CommonIdentity);
                var locationUri = linker.GetUriByName(http, "GetTechTagModelByID", new { id = model.CommonIdentity });
                return Results.Created(locationUri, succ);
            },
            fail =>
            {
                Log.Error(fail.ToException(), "Server issue encountered while trying to add a new TechTag Model to the database");
