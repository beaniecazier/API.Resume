...
/// </summary>
[ApiVersion(1.0)]
public class DeleteOrganizationModelEndpoint : IEndpoints
{
    ...
    {
        // Delete Endpoints
        Log.Information("Now adding Organization Model Delete Endpoints");
        var singleEndpoint = app.MapDelete(OrganizationModelEndpoints.Delete, DeleteAsync)
            .WithName("DeleteOrganizationModel")
    ...
    /// <summary>
    /// Delete an Organization Model by its id along with all of its history
    /// </summary>
    /// <param name="id">The id of the Organization Model to delete form the database</param>
    ...
    private static async Task<IResult> DeleteAsync(int id, IOrganizationModelService service,
        IOutputCacheStore outputCacheStore, CancellationToken token)
    {
        ...
        Log.Information("Delete Organization Model Endpoint called by {username}", @username);

        var entry = await service.DeleteAsync(id, token);
        if (!entry.IsFail) await outputCacheStore.EvictByTagAsync(OrganizationModelEndpoints.Tag, token);
        return entry.Match(
            Succ =>
            {
                Log.Information("Organization Model with id:{Succ.CommonIdentity}", @Succ.CommonIdentity);
            ...
            Fail =>
            {
                ...
                Log.Error(ex, $"Server issue encountered while trying to delete Organization Model with ID:{id} from the database");
