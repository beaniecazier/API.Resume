...
/// </summary>
[ApiVersion(1.0)]
public class DeleteCountryCodeModelEndpoint : IEndpoints
{
    ...
    {
        // Delete Endpoints
        Log.Information("Now adding CountryCode Model Delete Endpoints");
        var singleEndpoint = app.MapDelete(CountryCodeModelEndpoints.Delete, DeleteAsync)
            .WithName("DeleteCountryCodeModel")
    ...
    /// <summary>
    /// Delete an CountryCode Model by its id along with all of its history
    /// </summary>
    /// <param name="id">The id of the CountryCode Model to delete form the database</param>
    ...
    private static async Task<IResult> DeleteAsync(int id, ICountryCodeModelService service,
        IOutputCacheStore outputCacheStore, CancellationToken token)
    {
        ...
        Log.Information("Delete CountryCode Model Endpoint called by {username}", @username);

        var entry = await service.DeleteAsync(id, token);
        if (!entry.IsFail) await outputCacheStore.EvictByTagAsync(CountryCodeModelEndpoints.Tag, token);
        return entry.Match(
            Succ =>
            {
                Log.Information("CountryCode Model with id:{Succ.CommonIdentity}", @Succ.CommonIdentity);
            ...
            Fail =>
            {
                ...
                Log.Error(ex, $"Server issue encountered while trying to delete CountryCode Model with ID:{id} from the database");
