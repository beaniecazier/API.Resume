...
/// </summary>
[ApiVersion(1.0)]
public class DeleteAddressModelEndpoint : IEndpoints
{
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        var singleEndpoint = app.MapDelete(AddressModelEndpoints.Delete, DeleteAsync)
    ...
    private static async Task<IResult> DeleteAsync(int id, IAddressModelService service,
        IOutputCacheStore outputCacheStore, CancellationToken token)
    {
        ...
        Log.Information("Delete Address Model Endpoint called by {username}", @username);

        var entry = await service.DeleteAsync(id, token);
        if (!entry.IsFail) await outputCacheStore.EvictByTagAsync(AddressModelEndpoints.Tag, token);
        return entry.Match(
            Succ =>
            {
