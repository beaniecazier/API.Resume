...
/// </summary>
[ApiVersion(1.0)]
public class DeleteTechTagModelEndpoint : IEndpoints
{
    ...
    {
        // Delete Endpoints
        Log.Information("Now adding TechTag Model Delete Endpoints");
        var singleEndpoint = app.MapDelete(TechTagModelEndpoints.Delete, DeleteAsync)
    ...
    private static async Task<IResult> DeleteAsync(int id, ITechTagModelService service,
        IOutputCacheStore outputCacheStore, CancellationToken token)
    {
        ...
        Log.Information("Delete TechTag Model Endpoint called by {username}", @username);

        var entry = await service.DeleteAsync(id, token);
        if (!entry.IsFail) await outputCacheStore.EvictByTagAsync(TechTagModelEndpoints.Tag, token);
        return entry.Match(
            Succ =>
            {
                Log.Information("TechTag Model with id:{Succ.CommonIdentity}", @Succ.CommonIdentity);
            ...
            Fail =>
            {
                ...
                Log.Error(ex, $"Server issue encountered while trying to delete TechTag Model with ID:{id} from the database");
