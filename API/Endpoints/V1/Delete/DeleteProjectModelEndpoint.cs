...
public class DeleteProjectModelEndpoint : IEndpoints
{
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        var singleEndpoint = app.MapDelete(ProjectModelEndpoints.Delete, DeleteAsync)
    ...
    private static async Task<IResult> DeleteAsync(int id, IProjectModelService service,
        IOutputCacheStore outputCacheStore, CancellationToken token)
    {
        ...
        if (!entry.IsFail) await outputCacheStore.EvictByTagAsync(ProjectModelEndpoints.Tag, token);
        return entry.Match(
            Succ =>
            {
