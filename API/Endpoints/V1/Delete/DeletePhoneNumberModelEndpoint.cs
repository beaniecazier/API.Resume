...
/// </summary>
[ApiVersion(1.0)]
public class DeletePhoneNumberModelEndpoint : IEndpoints
{
    ...
    {
        // Delete Endpoints
        Log.Information("Now adding PhoneNumber Model Delete Endpoints");
        var singleEndpoint = app.MapDelete(PhoneNumberModelEndpoints.Delete, DeleteAsync)
            .WithName("DeletePhoneNumberModel")
    ...
    /// <summary>
    /// Delete an PhoneNumber Model by its id along with all of its history
    /// </summary>
    /// <param name="id">The id of the PhoneNumber Model to delete form the database</param>
    ...
    private static async Task<IResult> DeleteAsync(int id, IPhoneNumberModelService service,
        IOutputCacheStore outputCacheStore, CancellationToken token)
    {
        ...
        Log.Information("Delete PhoneNumber Model Endpoint called by {username}", @username);

        var entry = await service.DeleteAsync(id, token);
        if (!entry.IsFail) await outputCacheStore.EvictByTagAsync(PhoneNumberModelEndpoints.Tag, token);
        return entry.Match(
            Succ =>
            {
                Log.Information("PhoneNumber Model with id:{Succ.CommonIdentity}", @Succ.CommonIdentity);
            ...
            Fail =>
            {
                ...
                Log.Error(ex, $"Server issue encountered while trying to delete PhoneNumber Model with ID:{id} from the database");
