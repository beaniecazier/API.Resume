...
/// </summary>
[ApiVersion(1.0)]
public class DeleteContactModelEndpoint : IEndpoints
{
    ...
    {
        // Delete Endpoints
        Log.Information("Now adding Contact Model Delete Endpoints");
        var singleEndpoint = app.MapDelete(ContactModelEndpoints.Delete, DeleteAsync)
    ...
    /// <summary>
    /// Delete an Contact Model by its id along with all of its history
    /// </summary>
    /// <param name="id">The id of the Contact Model to delete form the database</param>
    ...
    private static async Task<IResult> DeleteAsync(int id, IContactModelService service,
        IOutputCacheStore outputCacheStore, CancellationToken token)
    {
        ...
        Log.Information("Delete Contact Model Endpoint called by {username}", @username);

        var entry = await service.DeleteAsync(id, token);
        if (!entry.IsFail) await outputCacheStore.EvictByTagAsync(ContactModelEndpoints.Tag, token);
        return entry.Match(
            Succ =>
            {
                Log.Information("Contact Model with id:{Succ.CommonIdentity}", @Succ.CommonIdentity);
            ...
            Fail =>
            {
                ...
                Log.Error(ex, $"Server issue encountered while trying to delete Contact Model with ID:{id} from the database");
