...
/// </summary>
[ApiVersion(1.0)]
public class DeleteEducationDegreeModelEndpoint : IEndpoints
{
    ...
    {
        // Delete Endpoints
        Log.Information("Now adding EducationDegree Model Delete Endpoints");
        var singleEndpoint = app.MapDelete(EducationDegreeModelEndpoints.Delete, DeleteAsync)
            .WithName("DeleteEducationDegreeModel")
    ...
    /// <summary>
    /// Delete an EducationDegree Model by its id along with all of its history
    /// </summary>
    /// <param name="id">The id of the EducationDegree Model to delete form the database</param>
    ...
    private static async Task<IResult> DeleteAsync(int id, IEducationDegreeModelService service,
        IOutputCacheStore outputCacheStore, CancellationToken token)
    {
        ...
        Log.Information("Delete EducationDegree Model Endpoint called by {username}", @username);

        var entry = await service.DeleteAsync(id, token);
        if (!entry.IsFail) await outputCacheStore.EvictByTagAsync(EducationDegreeModelEndpoints.Tag, token);
        return entry.Match(
            Succ =>
            {
                Log.Information("EducationDegree Model with id:{Succ.CommonIdentity}", @Succ.CommonIdentity);
            ...
            Fail =>
            {
                ...
                Log.Error(ex, $"Server issue encountered while trying to delete EducationDegree Model with ID:{id} from the database");
