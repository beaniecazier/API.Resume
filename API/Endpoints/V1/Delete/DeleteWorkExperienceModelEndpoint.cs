...
public class DeleteWorkExperienceModelEndpoint : IEndpoints
{
    ...
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        ...
        var singleEndpoint = app.MapDelete(WorkExperienceModelEndpoints.Delete, DeleteAsync)
        ...
        IOutputCacheStore outputCacheStore, CancellationToken token)
    {
        ...
        if (!entry.IsFail) await outputCacheStore.EvictByTagAsync(WorkExperienceModelEndpoints.Tag, token);
        return entry.Match(
            Succ =>
            {
