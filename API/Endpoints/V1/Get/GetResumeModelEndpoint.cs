...
/// </summary>
[ApiVersion(1.0)]
public class GetResumeModelEndpoint : IEndpoints
{
    ...
    {
        // Read Endpoints
        var singleEndpoint = app.MapGet(ResumeModelEndpoints.GetById, GetResumeModelByIDAsync)
            ...
            .HasApiVersion(1.0)
            .CacheOutput(ResumeModelEndpoints.Tag)
            .WithTags(ResumeModelEndpoints.Tag);

        var multipleEndpoint = app.MapGet(ResumeModelEndpoints.GetAll, GetAllResumeModelsAsync)
    ...
    private static async Task<IResult> GetResumeModelByIDAsync(int id, IResumeModelService service, CancellationToken token)
    {
        //string username = http.User.Identity!.Name??"fuck me....";
        string username = "Tiabeanie";

        Log.Information("Get Resume Model Endpoint called with id by {username}", @username);
        ...
        return entry.Match(
            ...
            Fail =>
            {
                ...
                Log.Error(ex, $"Server issue encountered while trying to get Resume Model with ID:{id} from the database");
        ...
        CancellationToken token)
    {
        ...
        return entries.Match(
            Succ =>
            {
                var responsesCollection = new ResumeModelsResponse()
                {
