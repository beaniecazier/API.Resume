...
public class GetPhoneNumberModelEndpoint : IEndpoints
{
    ...
    /// <response code="500">Something went wrong or the database does not exist</response>
    private static async Task<IResult> GetAllPhoneNumberModelsAsync(IPhoneNumberModelService service,
        [AsParameters] GetAllPhoneNumberModelsRequest searchParams,
        HttpContext context,
        CancellationToken token)
    {
        ...
        return entries.Match(
            Succ =>
            {
                var responsesCollection = new PhoneNumberModelsResponse()
                {
            ...
            Fail =>
            {
                Log.Error(Fail.ToException(), "Server issue encountered while trying to get all PhoneNumber Models from the database");
