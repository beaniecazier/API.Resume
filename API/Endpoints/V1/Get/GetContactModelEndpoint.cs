...
/// </summary>
[ApiVersion(1.0)]
public class GetContactModelEndpoint : IEndpoints
{
    ...
    {
        // Read Endpoints
        var singleEndpoint = app.MapGet(ContactModelEndpoints.GetById, GetContactModelByIDAsync)
            ...
            .HasApiVersion(1.0)
            .CacheOutput(ContactModelEndpoints.Tag)
            .WithTags(ContactModelEndpoints.Tag);

        var multipleEndpoint = app.MapGet(ContactModelEndpoints.GetAll, GetAllContactModelsAsync)
    ...
    private static async Task<IResult> GetContactModelByIDAsync(int id, IContactModelService service, CancellationToken token)
    {
        //string username = http.User.Identity!.Name??"fuck me....";
        string username = "Tiabeanie";

        Log.Information("Get Contact Model Endpoint called with id by {username}", @username);
        ...
        return entry.Match(
            ...
            Fail =>
            {
                ...
                {
                    Log.Error(ex, "Contact Model with ID:{id} does not exist", @id);
                    return Results.NotFound();
                }
                Log.Error(ex, $"Server issue encountered while trying to get Contact Model with ID:{id} from the database");
                return Results.Problem(detail: ex.ToString(), statusCode: StatusCodes.Status500InternalServerError);
            });
    }

    /// <summary>
    /// Retrieve all Contact Models from the database
    /// </summary>
    /// <param name="service">The service class the serves this Endpoint for database operations</param>
    /// <param name="context">The http client context</param>
    /// <param name="searchParams"></param>
    /// <param name="token">Cancelation token</param>
    /// <returns>A list of all Contact Models in the database</returns>
    /// <response code="200">Get all successful</response>
    /// <response code="500">Something went wrong or the database does not exist</response>
    private static async Task<IResult> GetAllContactModelsAsync(IContactModelService service,
        [AsParameters] GetAllContactModelsRequest searchParams,
        HttpContext context,
        CancellationToken token)
    {
        ...
        Log.Information("Get All Contact Models Endpoint called by {username}", @username);
        ...
        return entries.Match(
            Succ =>
            {
                var responsesCollection = new ContactModelsResponse()
                {
            ...
            Fail =>
            {
                Log.Error(Fail.ToException(), "Server issue encountered while trying to get all Contact Models from the database");
