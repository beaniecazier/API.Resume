...
/// </summary>
[ApiVersion(1.0)]
public class GetTechTagModelEndpoint : IEndpoints
{
    ...
    {
        // Read Endpoints
        var singleEndpoint = app.MapGet(TechTagModelEndpoints.GetById, GetTechTagModelByIDAsync)
            ...
            .HasApiVersion(1.0)
            .CacheOutput(TechTagModelEndpoints.Tag)
            .WithTags(TechTagModelEndpoints.Tag);

        var multipleEndpoint = app.MapGet(TechTagModelEndpoints.GetAll, GetAllTechTagModelsAsync)
    ...
    private static async Task<IResult> GetTechTagModelByIDAsync(int id, ITechTagModelService service, CancellationToken token)
    {
        //string username = http.User.Identity!.Name??"fuck me....";
        string username = "Tiabeanie";

        Log.Information("Get TechTag Model Endpoint called with id by {username}", @username);
        ...
        return entry.Match(
            ...
            Fail =>
            {
                ...
                {
                    Log.Error(ex, "TechTag Model with ID:{id} does not exist", @id);
                    return Results.NotFound();
                }
                Log.Error(ex, $"Server issue encountered while trying to get TechTag Model with ID:{id} from the database");
    ...
    /// <param name="token">Cancelation token</param>
    /// <returns>A list of all TechTag Models in the database</returns>
    /// <response code="200">Get all successful</response>
    /// <response code="500">Something went wrong or the database does not exist</response>
    private static async Task<IResult> GetAllTechTagModelsAsync(ITechTagModelService service,
        [AsParameters] GetAllTechTagModelsRequest searchParams,
        HttpContext context,
        CancellationToken token)
    {
        ...
        Log.Information("Get All TechTag Models Endpoint called by {username}", @username);
        ...
        return entries.Match(
            Succ =>
            {
                var responsesCollection = new TechTagModelsResponse()
                {
            ...
            Fail =>
            {
                Log.Error(Fail.ToException(), "Server issue encountered while trying to get all TechTag Models from the database");
