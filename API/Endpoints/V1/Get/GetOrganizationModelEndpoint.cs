...
/// <summary>
/// The collection of Endpoints for the Organization Model in API
/// </summary>
[ApiVersion(1.0)]
public class GetOrganizationModelEndpoint : IEndpoints
{
    private const string ContentType = "application/json";

    /// <summary>
    /// Add the Organization Model Service to the DI container
    /// </summary>
    ...
    /// <summary>
    /// Map all Organization Model Endpoints with correct settings
    /// </summary>
    /// <param name="app"></param>
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        // Read Endpoints
        var singleEndpoint = app.MapGet(OrganizationModelEndpoints.GetById, GetOrganizationModelByIDAsync)
            .WithName("GetOrganizationModelByID")
            .Produces(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status404NotFound)                                        // could not find result to update
            .Produces(StatusCodes.Status500InternalServerError)
            .WithApiVersionSet(APIVersioning.VersionSet)
            .HasApiVersion(1.0)
            .CacheOutput(OrganizationModelEndpoints.Tag)
            .WithTags(OrganizationModelEndpoints.Tag);

        var multipleEndpoint = app.MapGet(OrganizationModelEndpoints.GetAll, GetAllOrganizationModelsAsync)
            .WithName("GetAllOrganizationModels")
            .Produces(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status500InternalServerError)
            .WithApiVersionSet(APIVersioning.VersionSet)
            .HasApiVersion(1.0)
    ...
    private static async Task<IResult> GetOrganizationModelByIDAsync(int id, IOrganizationModelService service, CancellationToken token)
    {
        //string username = http.User.Identity!.Name??"fuck me....";
        string username = "Tiabeanie";

        Log.Information("Get Organization Model Endpoint called with id by {username}", @username);
        ...
        return entry.Match(
            ...
            Fail =>
            {
                ...
                {
                    Log.Error(ex, "Organization Model with ID:{id} does not exist", @id);
                    return Results.NotFound();
                }
                Log.Error(ex, $"Server issue encountered while trying to get Organization Model with ID:{id} from the database");
                return Results.Problem(detail: ex.ToString(), statusCode: StatusCodes.Status500InternalServerError);
            });
    }

    /// <summary>
    /// Retrieve all Organization Models from the database
    /// </summary>
    /// <param name="service">The service class the serves this Endpoint for database operations</param>
    /// <param name="context">The http client context</param>
    /// <param name="searchParams"></param>
    /// <param name="token">Cancelation token</param>
    /// <returns>A list of all Organization Models in the database</returns>
    /// <response code="200">Get all successful</response>
    /// <response code="500">Something went wrong or the database does not exist</response>
    private static async Task<IResult> GetAllOrganizationModelsAsync(IOrganizationModelService service,
        [AsParameters] GetAllOrganizationModelsRequest searchParams,
        HttpContext context,
        CancellationToken token)
    {
        ...
        Log.Information("Get All Organization Models Endpoint called by {username}", @username);
        ...
        return entries.Match(
            Succ =>
            {
                var responsesCollection = new OrganizationModelsResponse()
                {
            ...
            Fail =>
            {
                Log.Error(Fail.ToException(), "Server issue encountered while trying to get all Organization Models from the database");
