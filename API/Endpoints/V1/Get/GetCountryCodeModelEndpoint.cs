...
/// <summary>
/// The collection of Endpoints for the CountryCode Model in API
/// </summary>
[ApiVersion(1.0)]
public class GetCountryCodeModelEndpoint : IEndpoints
{
    private const string ContentType = "application/json";

    /// <summary>
    /// Add the CountryCode Model Service to the DI container
    /// </summary>
    ...
    /// <summary>
    /// Map all CountryCode Model Endpoints with correct settings
    /// </summary>
    /// <param name="app"></param>
    public static void DefineEndpoints(IEndpointRouteBuilder app)
    {
        // Read Endpoints
        var singleEndpoint = app.MapGet(CountryCodeModelEndpoints.GetById, GetCountryCodeModelByIDAsync)
            .WithName("GetCountryCodeModelByID")
            .Produces(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status404NotFound)                                        // could not find result to update
            .Produces(StatusCodes.Status500InternalServerError)
            .WithApiVersionSet(APIVersioning.VersionSet)
            .HasApiVersion(1.0)
            .CacheOutput(CountryCodeModelEndpoints.Tag)
            .WithTags(CountryCodeModelEndpoints.Tag);

        var multipleEndpoint = app.MapGet(CountryCodeModelEndpoints.GetAll, GetAllCountryCodeModelsAsync)
            .WithName("GetAllCountryCodeModels")
            .Produces(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status500InternalServerError)
            .WithApiVersionSet(APIVersioning.VersionSet)
            .HasApiVersion(1.0)
    ...
    private static async Task<IResult> GetCountryCodeModelByIDAsync(int id, ICountryCodeModelService service, CancellationToken token)
    {
        //string username = http.User.Identity!.Name??"fuck me....";
        string username = "Tiabeanie";

        Log.Information("Get CountryCode Model Endpoint called with id by {username}", @username);
        ...
        return entry.Match(
            ...
            Fail =>
            {
                ...
                {
                    Log.Error(ex, "CountryCode Model with ID:{id} does not exist", @id);
                    return Results.NotFound();
                }
                Log.Error(ex, $"Server issue encountered while trying to get CountryCode Model with ID:{id} from the database");
                return Results.Problem(detail: ex.ToString(), statusCode: StatusCodes.Status500InternalServerError);
            });
    }

    /// <summary>
    /// Retrieve all CountryCode Models from the database
    /// </summary>
    /// <param name="service">The service class the serves this Endpoint for database operations</param>
    /// <param name="context">The http client context</param>
    /// <param name="searchParams"></param>
    /// <param name="token">Cancelation token</param>
    /// <returns>A list of all CountryCode Models in the database</returns>
    /// <response code="200">Get all successful</response>
    /// <response code="500">Something went wrong or the database does not exist</response>
    private static async Task<IResult> GetAllCountryCodeModelsAsync(ICountryCodeModelService service,
        [AsParameters] GetAllCountryCodeModelsRequest searchParams,
        HttpContext context,
        CancellationToken token)
    {
        ...
        Log.Information("Get All CountryCode Models Endpoint called by {username}", @username);
        ...
        return entries.Match(
            Succ =>
            {
                var responsesCollection = new CountryCodeModelsResponse()
                {
            ...
            Fail =>
            {
                Log.Error(Fail.ToException(), "Server issue encountered while trying to get all CountryCode Models from the database");
