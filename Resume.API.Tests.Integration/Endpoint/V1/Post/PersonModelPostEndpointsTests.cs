...
public class PersonModelPostEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<int> _createdPersonModels = new List<int>();

    public PersonModelPostEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task CreatePersonModel_CreatesPersonModel_WhenDataIsCorrect()
    {
        ...
        // ACT
        var result = await httpClient.PostAsJsonAsync(PersonModelEndpoints.Post, modelRequest);
        ...
        result.Headers.Location.AbsolutePath.Should().Be($"/{PersonModelEndpoints.EndpointPrefix}/{createdModel.Id}");
    }
