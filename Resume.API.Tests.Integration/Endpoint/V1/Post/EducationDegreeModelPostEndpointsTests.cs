...
public class EducationDegreeModelPostEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<int> _createdEducationDegreeModels = new List<int>();

    public EducationDegreeModelPostEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task CreateEducationDegreeModel_CreatesEducationDegreeModel_WhenDataIsCorrect()
    {
        ...
        // ACT
        var result = await httpClient.PostAsJsonAsync(EducationDegreeModelEndpoints.Post, modelRequest);
        ...
        result.Headers.Location.AbsolutePath.Should().Be($"/{EducationDegreeModelEndpoints.EndpointPrefix}/{createdModel.Id}");
    }
