...
public class OrganizationModelPostEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<int> _createdOrganizationModels = new List<int>();

    public OrganizationModelPostEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    [Fact]
    public async Task CreateOrganizationModel_CreatesOrganizationModel_WhenDataIsCorrect()
    {
        // ARRANGE
        var httpClient = _factory.CreateClient();
        ...
        // ACT
        var result = await httpClient.PostAsJsonAsync(OrganizationModelEndpoints.Post, modelRequest);
        ...
        result.Headers.Location.AbsolutePath.Should().Be($"/{OrganizationModelEndpoints.EndpointPrefix}/{createdModel.Id}");
    }

    //[Fact]
    //public async Task CreateOrganizationModel_Fails_WhenHouseNumberIsInvalid()
    //{
    //    // ARRANGE
    //    var httpClient = _factory.CreateClient();
    ...
    //[Fact]
    //public async Task CreateOrganizationModel_Fails_InternalServerError() => Task.CompletedTask;
}
