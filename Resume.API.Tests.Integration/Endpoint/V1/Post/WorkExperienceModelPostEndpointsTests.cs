...
public class WorkExperienceModelPostEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<int> _createdWorkExperienceModels = new List<int>();

    public WorkExperienceModelPostEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task CreateWorkExperienceModel_CreatesWorkExperienceModel_WhenDataIsCorrect()
    {
        ...
        // ACT
        var result = await httpClient.PostAsJsonAsync(WorkExperienceModelEndpoints.Post, modelRequest);
        ...
        result.Headers.Location.AbsolutePath.Should().Be($"/{WorkExperienceModelEndpoints.EndpointPrefix}/{createdModel.Id}");
    }
