...
public class CertificateModelPostEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<int> _createdCertificateModels = new List<int>();

    public CertificateModelPostEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task CreateCertificateModel_CreatesCertificateModel_WhenDataIsCorrect()
    {
        ...
        // ACT
        var result = await httpClient.PostAsJsonAsync(CertificateModelEndpoints.Post, modelRequest);
        ...
        result.Headers.Location.AbsolutePath.Should().Be($"/{CertificateModelEndpoints.EndpointPrefix}/{createdModel.Id}");
    }
