...
public class ResumeModelGetEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<int> _createdResumeModels = new List<int>();

    public ResumeModelGetEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task GetResumeModel_ReturnsResumeModel_WhenModelExists()
    {
        ...
        // ACT
        var result = await httpClient.GetAsync($"{ResumeModelEndpoints.EndpointPrefix}/{createdModel.Id}");
    ...
    public async Task GetResumeModel_ReturnsNotFound_WhenModelDoesNotExist()
    {
        ...
        // ACT
        var result = await httpClient.GetAsync($"{ResumeModelEndpoints.EndpointPrefix}/{-1000000}");
    ...
    public async Task GetAllResumeModels_ReturnsAllModelsUnsorted_WhenModelsExist()
    {
        ...
        var result = await httpClient.GetAsync($"{ResumeModelEndpoints.EndpointPrefix}?{searchTerms}");
