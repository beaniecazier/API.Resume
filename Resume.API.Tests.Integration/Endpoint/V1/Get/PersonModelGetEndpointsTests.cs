...
public class PersonModelGetEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<int> _createdPersonModels = new List<int>();

    public PersonModelGetEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task GetPersonModel_ReturnsPersonModel_WhenModelExists()
    {
        ...
        // ACT
        var result = await httpClient.GetAsync($"{PersonModelEndpoints.EndpointPrefix}/{createdModel.Id}");
    ...
    public async Task GetPersonModel_ReturnsNotFound_WhenModelDoesNotExist()
    {
        ...
        // ACT
        var result = await httpClient.GetAsync($"{PersonModelEndpoints.EndpointPrefix}/{-1000000}");
    ...
    public async Task GetAllPersonModels_ReturnsAllModelsUnsorted_WhenModelsExist()
    {
        ...
        var result = await httpClient.GetAsync($"{PersonModelEndpoints.EndpointPrefix}?{searchTerms}");
