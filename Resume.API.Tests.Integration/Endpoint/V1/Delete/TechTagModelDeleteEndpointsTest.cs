...
public class TechTagModelDeleteEndpointsTest : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<TechTagModelResponse> _createdTechTagModels = new List<TechTagModelResponse>();

    public TechTagModelDeleteEndpointsTest(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task DeleteTechTagModel_ReturnsOk_WhenModelDoesExist()
    {
        ...
        // ACT
        var result = await httpClient.DeleteAsync($"{TechTagModelEndpoints.EndpointPrefix}/{createdResponse.Id}");
    ...
    public async Task DeleteTechTagModel_ReturnsNotFound_WhenModelDoesNotExist()
    {
        ...
        // ACT
        var result = await httpClient.DeleteAsync($"{TechTagModelEndpoints.EndpointPrefix}/{-1000000}");
