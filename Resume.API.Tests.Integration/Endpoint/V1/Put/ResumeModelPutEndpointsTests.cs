...
public class ResumeModelPutEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<ResumeModelResponse> _createdResumeModels = new List<ResumeModelResponse>();

    public ResumeModelPutEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task UpdateResumeModel_UpdatesModel_WhenDataIsCorrect()
    {
		...
		updatedModel.HeroStatement.Should().Be(createdResponse.HeroStatement);
		updatedModel.Degrees.Should().BeEquivalentTo(createdResponse.Degrees.Select(x=>x.Id).ToArray());
		updatedModel.Certificates.Should().BeEquivalentTo(createdResponse.Certificates.Select(x=>x.Id).ToArray());
		updatedModel.WorkExperience.Should().BeEquivalentTo(createdResponse.WorkExperience.Select(x=>x.Id).ToArray());
		updatedModel.Projects.Should().BeEquivalentTo(createdResponse.Projects.Select(x=>x.Id).ToArray());
		updatedModel.PreferedName.Should().Be(createdResponse.PreferedName);
		updatedModel.Pronouns.Should().BeEquivalentTo(createdResponse.Pronouns);
		updatedModel.Addresses.Should().BeEquivalentTo(createdResponse.Addresses.Select(x=>x.Id).ToArray());
		...
		updatedModel.PhoneNumbers.Should().BeEquivalentTo(createdResponse.PhoneNumbers.Select(x=>x.Id).ToArray());
    }
    ...
    public async Task UpdateResumeModel_ReturnsNotFound_WhenModelDoesNotExist()
    {
        ...
        var fakedModel = modelRequest.MapToModelFromCreateRequest(-10000,"IntegrationTesting",
	        new List<EducationDegreeModel>(),
	        new List<CertificateModel>(),
	        new List<WorkExperienceModel>(),
	        new List<ProjectModel>(),
	        new List<AddressModel>(),
	        new List<PhoneNumberModel>());
