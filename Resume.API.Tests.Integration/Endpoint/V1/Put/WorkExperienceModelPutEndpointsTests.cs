...
public class WorkExperienceModelPutEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<WorkExperienceModelResponse> _createdWorkExperienceModels = new List<WorkExperienceModelResponse>();

    public WorkExperienceModelPutEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task UpdateWorkExperienceModel_UpdatesModel_WhenDataIsCorrect()
    {
        ...
        var result = await httpClient.PutAsJsonAsync($"{WorkExperienceModelEndpoints.EndpointPrefix}/{createdResponse.Id}", updateRequest);
        ...
        result.Headers.Location.AbsolutePath.Should().Be($"/{WorkExperienceModelEndpoints.EndpointPrefix}/{updatedModel.Id}");

        updatedModel.Id.Should().Be(createdResponse.Id);
        updatedModel.Name.Should().NotBe(createdResponse.Name);

		updatedModel.StartDate.Should().Be(createdResponse.StartDate);
		updatedModel.EndDate.Should().Be(createdResponse.EndDate);
		updatedModel.Company.Should().Be(createdResponse.Company.Id);
		updatedModel.Description.Should().Be(createdResponse.Description);
		updatedModel.Responsibilities.Should().BeEquivalentTo(createdResponse.Responsibilities);
		updatedModel.TechUsed.Should().BeEquivalentTo(createdResponse.TechUsed.Select(x=>x.Id).ToArray());
		updatedModel.EmploymentType.Should().Be(createdResponse.EmploymentType);
    }
    ...
    //    var result = await httpClient.GetAsync($"{WorkExperienceModelEndpoints.EndpointPrefix}/{createdResponse.Id}");
    ...
    //    result.Headers.Location.Should().Be($"{WorkExperienceModelEndpoints.EndpointPrefix}/{updatedModel.Id}");
    //}
    ...
    public async Task UpdateWorkExperienceModel_ReturnsNotFound_WhenModelDoesNotExist()
    {
        ...
        var propRecord = await ModelGenerator.PopulateDatabaseForWorkExperienceModelTest(httpClient);
        var modelRequest = ModelGenerator.GenerateNewCreateWorkExperienceModelRequest(propRecord);
        var fakedModel = modelRequest.MapToModelFromCreateRequest(-10000,"IntegrationTesting", new OrganizationModel(), new List<TechTagModel>());
        ...
        var result = await httpClient.PutAsJsonAsync($"{WorkExperienceModelEndpoints.EndpointPrefix}/{updateRequest.Id}", updateRequest);
