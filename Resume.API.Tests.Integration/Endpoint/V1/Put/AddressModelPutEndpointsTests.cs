...
public class AddressModelPutEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<AddressModelResponse> _createdAddressModels = new List<AddressModelResponse>();

    public AddressModelPutEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task UpdateAddressModel_UpdatesModel_WhenDataIsCorrect()
    {
		...
		updatedModel.HouseNumber.Should().Be(createdResponse.HouseNumber);
		updatedModel.StreetName.Should().Be(createdResponse.StreetName);
		updatedModel.StreetType.Should().Be(createdResponse.StreetType);
		updatedModel.City.Should().Be(createdResponse.City);
		updatedModel.Region.Should().Be(createdResponse.Region);
		updatedModel.State.Should().Be(createdResponse.State);
		updatedModel.Country.Should().Be(createdResponse.Country);
		updatedModel.PostalCode.Should().Be(createdResponse.PostalCode);
		updatedModel.Zip4.Should().Be(createdResponse.Zip4);
		updatedModel.CrossStreetName.Should().Be(createdResponse.CrossStreetName);
		updatedModel.PrefixDirection.Should().Be(createdResponse.PrefixDirection);
		updatedModel.PrefixType.Should().Be(createdResponse.PrefixType);
		updatedModel.SuffixDirection.Should().Be(createdResponse.SuffixDirection);
		updatedModel.SuffixType.Should().Be(createdResponse.SuffixType);
    }
