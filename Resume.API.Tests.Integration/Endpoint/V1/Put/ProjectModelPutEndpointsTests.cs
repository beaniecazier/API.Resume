...
public class ProjectModelPutEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<ProjectModelResponse> _createdProjectModels = new List<ProjectModelResponse>();

    public ProjectModelPutEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task UpdateProjectModel_UpdatesModel_WhenDataIsCorrect()
    {
		...
		updatedModel.Version.Should().Be(createdResponse.Version);
		updatedModel.ProjectUrl.Should().Be(createdResponse.ProjectUrl);
		updatedModel.StartDate.Should().Be(createdResponse.StartDate);
		updatedModel.CompletionDate.Should().Be(createdResponse.CompletionDate);
		updatedModel.TechTags.Should().BeEquivalentTo(createdResponse.TechTags);
    }
