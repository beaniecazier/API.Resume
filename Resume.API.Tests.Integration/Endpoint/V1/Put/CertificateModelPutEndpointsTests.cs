...
public class CertificateModelPutEndpointsTests : IClassFixture<IntegrationTestWebApplicationFactory>, IAsyncLifetime
{
    private readonly IntegrationTestWebApplicationFactory _factory;

    private List<CertificateModelResponse> _createdCertificateModels = new List<CertificateModelResponse>();

    public CertificateModelPutEndpointsTests(IntegrationTestWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task UpdateCertificateModel_UpdatesModel_WhenDataIsCorrect()
    {
		...
		updatedModel.IssueDate.Should().Be(createdResponse.IssueDate);
		updatedModel.Link.Should().Be(createdResponse.Link);
		updatedModel.PdfFileName.Should().Be(createdResponse.PdfFileName);
		updatedModel.Issuer.Should().Be(createdResponse.Issuer.Id);
		updatedModel.CertificateID.Should().Be(createdResponse.CertificateID);
		updatedModel.ExpirationDate.Should().Be(createdResponse.ExpirationDate);
    }
